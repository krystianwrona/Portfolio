...
- Scroll rozsypuje ptaka (uScroll w shaderze). `src/lib/crowScroll.ts` powiela stałe eksplozji z shadera i liczy z nich opacity warstwy canvasu — ustawia je istniejący handler scrolla w page.tsx jako `--crow-opacity`. Zmiana stałych w shaderze wymaga zmiany w crowScroll.ts
